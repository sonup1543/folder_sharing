...
class FileManagerController extends Controller
{
    ...
    public function fileManagerss(){
        ...
        $folders = FileManager::where('parent_id', 0)->where('delete_at', 0)->get();
        return view('admin.file-manager.view', compact('title', 'employees', 'folders'));
    }
    ...
    public function storeFolder(Request $request)
    {
        $request->validate([
            ...
            'parent_id' => 'nullable|exists:file_managers,id',  // Ensure it references the correct table
        ...
        $existingFolder = FileManager::where('name', $folderName)
            ->where('parent_id', 0)  // Assuming '0' means it's a top-level folder
            ->where('delete_at', 0)  // Ensure it's not deleted
            ->exists();
        ...
        $folder = FileManager::create([
            'name' => $request->name,
            'parent_id' => 0,  // Assuming '0' for parent_id means top-level folder
            'created_by' => auth()->id(),
            'assigned_to' => $request->assigned_to ?? 0,  // Handle nullable assigned_to
            'path' => $folderPath,
        ]);
    
        // Assign the folder to a user, if assigned
        if ($request->assigned_to) {
            FolderUserAssignment::create([
                'folder_id' => $folder->id,  // Use $folder->id to reference the newly created folder
        ...
        public function folderView($folderName, $folder_id)
        {
            ...
            return view('admin.file-manager.view1', [
                'title' => $folder->name,
                'folders' => $folders,
                'folder_id' => $folder_id,
                'ancestors' => $ancestors,
...
public function assignList($folderId)
{
    $folder = FileManager::find($folderId);
    if (!$folder) {
        return redirect()->route('file-manager.index')->with('error', 'Folder not found');
    }

    // Get all users
    $users = Employee::all();

    // Get users assigned to the specific folder (using the folder_user_assignments table)
    $assignedUserIds = FolderUserAssignment::where('folder_id', $folderId)->pluck('user_id')->toArray();
    $assignedUsers = Employee::whereIn('id', $assignedUserIds)->get();

    return view('admin.file-manager.assign', compact('folder', 'users', 'assignedUsers'));
}

public function assignUsers(Request $request, $folderId)
{
    $folder = FileManager::find($folderId);
    if (!$folder) {
        return redirect()->route('file-manager.index')->with('error', 'Folder not found');
    ...
    $request->validate([
        ...
        'users.*' => 'exists:employees,id',
    ]);

    // Assign users to the folder (insert records in folder_user_assignments table)
    ...
    return redirect()->route('folder.assign.list', $folderId)->with('success', 'Users assigned successfully');
}

public function removeAssignment($folderId, $userId)
{
    // Remove the assignment (delete the record from folder_user_assignments table)
    FolderUserAssignment::where('folder_id', $folderId)
                         ->where('user_id', $userId)
                         ->delete();

    return redirect()->route('folder.assign.list', $folderId)->with('success', 'User assignment removed');
}
